...
class BillingManager(models.Manager):
	def new_or_get(self, request):	
		...
		# Guest user who logged in and has an account
		...
		# Signup user
			...
			bp_qs = self.model.objects.filter(user=request.user, email=request.user.email)
			if bp_qs.exists():
				billing_profile = bp_qs.last()
			else:
				billing_profile = self.model.objects.create(user=request.user, email=request.user.email)
				created = True
...
class Card:
	pass
	# Save all the data from the card response
	# Tie it to a Billing Profile Foriegn Key
	# Also add the stripe_id

class Charge:
	pass
	# Save all the data from the charge response
	# Tie it to a Billing Profile Foriegn Key
	# Also add the stripe_id