...
// Detail View
function detailAPIView(postID){
	let url = `http://127.0.0.1:8000/post_api/detail/${postID}`
	fetch(url)
	.then(res => res.json())
	.then(data => {
		...
		// We want to remove the link for detailview
		newPost.innerHTML =  `
			<div class="card-header">
				<p>${data.title}</p>
			...
			<div class="card-body">
				...
				<button type="button" class="btn btn-info jsUpdate ${data.id}" data-toggle="modal" data-target="#updatePostModal">Update Post</button>
			</div>`
		div.append(newPost)
...
function deleteAPIView(postID){
	...
	fetch(url, {
		...
		// We want to send the user back to the home page after the post is deleted
		window.location.href = '/'
}


// No longer need a create view (optional)
...
let postIdURL = document.URL.split('/')[4];
document.addEventListener('DOMContentLoaded',detailAPIView(postIdURL))
