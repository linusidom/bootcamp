...
def cart_checkout(request):
	try:
		del request.session['backup_order_id']
	except:
		pass
	old_addresses = None
	order_obj = None
...
def cart_success(request):
	# Check whether the requirements of the order are satisfied
	backup_order_id = request.session.get('backup_order_id')
	if backup_order_id:
		order_obj = Order.objects.get(id=backup_order_id)
	else:
		cart_obj, created = Cart.objects.new_or_get(request)
		billing_profile, billing_profile_created = BillingProfile.objects.new_or_get(request)
		order_obj, created = Order.objects.new_or_get(billing_profile, cart_obj)
	...
	if is_done:
		order_obj.mark_paid()
		request.session['backup_order_id'] = order_obj.id
	...
	# If check is done we mark the status to paid

	# We want to remember the order ID for future use
